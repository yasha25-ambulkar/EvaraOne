...
const Navbar = () => {
  ...
  const navItems = [
    ...
    ...(user?.role === "superadmin"
      ? [{ name: "SUPER ADMIN", path: "/superadmin", icon: Shield }]
      : []),
    // Show ADMINISTRATION for community_admin role only (not for customer)
    ...(user?.role === "community_admin"
      ? [{ name: "ADMINISTRATION", path: "/admin", icon: LayoutGrid }]
      : []),
  ];
    ...
    <div className="fixed top-2 left-0 right-0 z-[2000] flex justify-center px-4 md:px-12 pointer-events-none">
      ...
      >
        ...
        >
          ...
          />
          <span className="text-[22px] font-black hidden lg:block tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-[#0E79C1] via-[#00A3A6] to-[#2BC872]">
            EvaraTech
          </span>
        </div>
            ...
            return (
              <Link
                ...
                className={clsx(
                  ...
                  isActive
                    ? "navbar-active"
                    : "px-4 py-2 rounded-full hover:bg-black/5 dark:hover:bg-white/5 navbar-text",
                )}
              >
                <item.icon
                  size={18}
          ...
          {isAuthenticated && user ? (
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-3">
                <div className="w-[40px] h-[40px] rounded-full flex items-center justify-center font-bold text-[16px] bg-blue-100 text-blue-900 dark:bg-blue-900/40 dark:text-white shadow-sm border border-white/50">
