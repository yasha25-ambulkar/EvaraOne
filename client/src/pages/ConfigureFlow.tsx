...
const ConfigureFlow = () => {
    ...
    const [waterSecurity, setWaterSecurity] = useState(true);
    const [systemDynamics, setSystemDynamics] = useState(true);
    const [alerts, setAlerts] = useState(false);
    const [consumptionPattern, setConsumptionPattern] = useState(true);
                ...
                <div className="max-w-[1000px] mx-auto flex flex-col gap-4">
                    ...
                    <div className="apple-glass-card rounded-[2rem] p-8 md:p-10" style={{
                        ...
                        <div className="flex flex-col gap-7">
                            ...
                            <div className="flex justify-between items-center group">
                                <div className="flex flex-col">
                                    <h4 className="text-[15px] font-bold m-0" style={{ color: '#1c2b4f' }}>Water Security Monitoring</h4>
                                    <p className="text-[13px] font-medium m-0 mt-1.5" style={{ color: '#64748b' }}>Track leak detection, pressure, and backflow risk</p>
                                </div>
                                <ToggleSwitch checked={waterSecurity} onChange={() => setWaterSecurity(!waterSecurity)} />
                            </div>

                            {/* Parameter 3 */}
                            <div className="flex justify-between items-center group">
                                <div className="flex flex-col">
                                    <h4 className="text-[15px] font-bold m-0" style={{ color: '#1c2b4f' }}>System Dynamics</h4>
                                    <p className="text-[13px] font-medium m-0 mt-1.5" style={{ color: '#64748b' }}>Monitor flow velocity, pressure trends, and pump state</p>
                                </div>
                                <ToggleSwitch checked={systemDynamics} onChange={() => setSystemDynamics(!systemDynamics)} />
                            </div>

                            {/* Parameter 4 */}
                            <div className="flex justify-between items-center group">
                                <div className="flex flex-col">
                                    <h4 className="text-[15px] font-bold m-0" style={{ color: '#1c2b4f' }}>Alerts</h4>
                                    <p className="text-[13px] font-medium m-0 mt-1.5" style={{ color: '#64748b' }}>System warnings and real-time flow notifications</p>
                                </div>
                                <ToggleSwitch checked={alerts} onChange={() => setAlerts(!alerts)} />
                            </div>

                            {/* Parameter 5 */}
                            <div className="flex justify-between items-center pb-2">
                                <div className="flex flex-col">
                                    <h4 className="text-[15px] font-bold m-0" style={{ color: '#1c2b4f' }}>Consumption Pattern</h4>
                                    <p className="text-[13px] font-medium m-0 mt-1.5" style={{ color: '#64748b' }}>Analyse daily and historical water usage trends</p>
                                </div>
                                <ToggleSwitch checked={consumptionPattern} onChange={() => setConsumptionPattern(!consumptionPattern)} />
                            </div>
