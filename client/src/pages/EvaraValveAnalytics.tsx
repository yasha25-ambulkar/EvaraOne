...
const ValveLimitPanel = ({
  ...
  // Valve state
  const [status, setStatus] = useState<"OPEN" | "CLOSED" | "TRANSITIONING">(
    "CLOSED",
  );
  ...
  const [displayDeliveredVolume, setDisplayDeliveredVolume] =
    useState<number>(0);
  ...
  const liveEstimateTimerRef = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (session.valveOpen) {
      ...
      startPolling(
        session.startVolume,
        session.shutoffLimit,
        session.deliveredVolume,
      );
    }
  ...
  const fetchCurrentTotalVolume = async (): Promise<number | null> => {
    if (
      typeof currentTotalVolume === "number" &&
      Number.isFinite(currentTotalVolume)
    ) {
  ...
  const sendValveCommand = async (
    action: "OPEN" | "CLOSED",
    autoShutoffLimit?: number,
  ): Promise<boolean> => {
    if (!hardwareId) return false;

    await api.post(`/nodes/${hardwareId}/command`, {
      action,
      ...(typeof autoShutoffLimit === "number" ? { autoShutoffLimit } : {}),
    });

    return true;
  };

  // ── Write CLOSED through backend with retry (S15) ────────────────────────
  const writeClosedToFirebase = async (retries = 3): Promise<boolean> => {
    for (let i = 0; i < retries; i++) {
      try {
        await sendValveCommand("CLOSED");
        return true;
      } catch {
  ...
  const startPolling = (startVolume: number, limit: number, resumeFrom = 0) => {
    ...
    pollIntervalRef.current = setInterval(async () => {
      ...
      if (current === null) {
        ...
        if (pollFailCountRef.current >= MAX_POLL_FAILURES) {
          ...
          if (closed) {
            setStatus("CLOSED");
            setValveError(
              `ThingSpeak unreachable after ${MAX_POLL_FAILURES} attempts. Valve closed for safety.`,
            );
            ...
            setValveError(
              "ThingSpeak AND Firebase unreachable. Check connections immediately.",
            );
          }
        }
        return;
      ...
      const delivered = Math.max(
        resumeFrom,
        Math.max(0, current - startVolume),
      );
  ...
  const closeValveAtLimit = async (
    startVolume: number,
    limit: number,
    deliveredAtClose: number,
  ) => {
  ...
  // Gauge calculation
  const gaugeMax =
    shutoffLimit != null && shutoffLimit > 0
      ? shutoffLimit
      : DEFAULT_SHUTOFF_LIMIT_L;
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const elapsedMinutes = Math.max(
        0,
        (Date.now() - lastSnapshotAtRef.current) / 60000,
      );
      const estimated = actual + elapsedMinutes * rate;
      setDisplayDeliveredVolume(
        Math.min(gaugeMax, Math.max(actual, estimated)),
      );
    };
  ...
  const percentage =
    gaugeMax > 0 ? Math.min(100, (liveDeliveredVolume / gaugeMax) * 100) : 0;
  ...
  useEffect(() => {
    ...
    void closeValveAtLimit(
      startVolumeRef.current,
      gaugeMax,
      liveDeliveredVolume,
    );
  }, [gaugeMax, liveDeliveredVolume, status]);

  // ── Persist limit through backend ─────────────────────────────────────────
  const persistShutoffLimit = async (limit: number): Promise<boolean> => {
    if (!hardwareId) {
      setLimitError("Device ID not available");
      return false;
    }
    setIsSavingLimit(true);
    setLimitError(null);
    try {
      await sendValveCommand(status === "OPEN" ? "OPEN" : "CLOSED", limit);
      onLimitSaved?.(limit);
      return true;
    } catch (err) {
  ...
  const handleSetLimit = async () => {
    ...
    if (status === "OPEN") {
      setLimitError(
        "Valve is already open. Close it first before setting a new limit.",
      );
      return;
    }
    ...
    if (snapshotVolume === null) {
      setLimitError(
        "Could not read ThingSpeak after 3 attempts. Check channel ID and read key.",
      );
      return;
    }
  ...
  const openValveInternal = async (limit: number, snapshotVolume: number) => {
    if (!hardwareId) {
      setValveError("Device ID not available");
      return;
    }
    setStatus("TRANSITIONING");
    setValveError(null);
    try {
      await sendValveCommand("OPEN", limit);
  ...
  const handleValveControl = async (newStatus: "OPEN" | "CLOSED") => {
    ...
    if (newStatus === "CLOSED") {
      ...
      try {
        ...
        const msg =
          err instanceof Error ? err.message : "Failed to close valve";
        setValveError(msg);
        setStatus("OPEN");
      }
      return;
  ...
  return (
    <div className="apple-glass-card rounded-[2rem] p-6 flex flex-col gap-5 h-full">
      <h3
        className="text-[13px] font-black uppercase tracking-widest m-0"
      ...
      <div className="flex flex-col items-center justify-center">
        <div className="relative w-48 h-48 flex items-center justify-center">
          ...
          >
            ...
            <circle
              ...
              r={radius}
              stroke={
                percentage >= 100 ? "#ef4444" : "url(#gaugeGradPanelBlue)"
              }
              strokeWidth={strokeWidth}
              fill="transparent"
              strokeDasharray={`${arcLength} ${circumference}`}
        ...
        {pollFailCountRef.current > 0 &&
          pollFailCountRef.current < MAX_POLL_FAILURES && (
            <p className="text-[9px] font-bold text-amber-500 mt-1">
              ThingSpeak fetch failed ({pollFailCountRef.current}/
              {MAX_POLL_FAILURES})...
            </p>
          )}
      ...
      <div className="flex flex-col gap-2">
        ...
        <input
          ...
          value={shutoffInput}
          onChange={(e) => {
            setShutoffInput(e.target.value);
            setLimitError(null);
          }}
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={
              isSavingLimit || status === "TRANSITIONING" || status === "OPEN"
            }
        ...
        <p className="text-[9px] text-slate-400 leading-relaxed">
          Enter limit → click Set & Open. Valve auto-closes when delivered
          litres reach the limit.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            <Play
              size={11}
              className="fill-current group-hover:scale-125 transition-transform duration-300"
            />
            Open
...
const EvaraValveAnalytics = () => {
  ...
  const tsChannel =
    ...
    deviceConfig?.thingspeakChannelId ||
    "";
  const positionField =
    deviceConfig?.position_field || deviceConfig?.positionField || "";
  const statusField =
    deviceConfig?.status_field || deviceConfig?.statusField || "";
  const flowField =
    deviceConfig?.flow_field ||
    deviceConfig?.flowField ||
    deviceConfig?.fields?.flow ||
    "field2";
  const totalVolumeField =
    deviceConfig?.total_volume_field ||
    deviceConfig?.totalVolumeField ||
    deviceConfig?.fields?.total_volume ||
    deviceConfig?.fields?.totalVolume ||
    "field1";
  const flowFieldName =
    deviceConfig?.flow_field_name ||
    deviceConfig?.flowFieldName ||
    deviceConfig?.fields?.flow_name ||
    deviceConfig?.fields?.flowName ||
    "";
  const totalVolumeFieldName =
    deviceConfig?.total_volume_field_name ||
    deviceConfig?.totalVolumeFieldName ||
    deviceConfig?.fields?.total_volume_name ||
    "";

  const selectedFlowField = useMemo(() => {
    const normalized = String(flowField || "")
      .trim()
      .toLowerCase();
  ...
  const totalVolumeFieldCandidates = useMemo(() => {
    const normalized = String(totalVolumeField || "")
      .trim()
      .toLowerCase();
  ...
  const tsReadKey =
    ...
    deviceConfig?.thingspeakReadKey ||
    undefined;
  ...
  useEffect(() => {
    ...
    flowFieldName,
    totalVolumeField,
    totalVolumeFieldName,
  ]);

  const latestTelemetry = useMemo(() => {
    if (!tsLatest?.values)
      return { flow: null, totalVolume: null, status: null };
    return {
  ...
  const [backfilledReadings, setBackfilledReadings] = useState<any[] | null>(
    null,
  );

  useEffect(() => {
    const valid = (tsReadings || [])
      .map((r) => {
    ...
    (async () => {
      try {
        ...
        const mapped = feeds
          ...
          .map((f: any) => ({
            timestamp: new Date(
              /Z|\+|\-/.test(f.created_at) ? f.created_at : f.created_at + "Z",
            ).toISOString(),
            entry_id: f.entry_id,
            values: tsFields.reduce((acc: any, fk: string) => {
              acc[fk] = f[fk] ?? null;
              return acc;
            }, {}),
          }));
  ...
  const flowHistory = useMemo(() => {
    const source =
      (backfilledReadings && Array.isArray(backfilledReadings)
        ? backfilledReadings
        : tsReadings) || [];
    const mapped = source
      .map((reading: any) => {
    ...
    const deduped = mapped.filter(
      (point: any, idx: number, arr: any[]) =>
        idx === 0 || point.ts !== arr[idx - 1].ts,
    );
  ...
  const totalVolumeValue = (() => {
    const v =
      latestTelemetry.totalVolume ??
      unifiedData?.latest?.total_liters ??
      unifiedData?.latest?.volume ??
      null;
  ...
  const savedShutoffLimit = useMemo(() => {
    const raw =
      deviceConfig?.auto_shutoff_limit ?? deviceConfig?.autoShutoffLimit;
  ...
  return (
    <div
      className="min-h-screen bg-transparent font-sans relative overflow-x-hidden"
      style={{ color: "var(--text-primary)" }}
    >
      <main
        ...
        <div className="max-w-[1400px] mx-auto w-full relative z-10 flex-1 flex flex-col">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-4">
            ...
            <div className="flex items-center gap-2 flex-wrap pb-1 md:self-end lg:self-auto">
              <div
                className={isOnline ? "pill-button green" : "pill-button red"}
              >
                <span
              ...
              <button
                ...
                className="pill-button purple active:scale-95"
              >
                <span className="material-icons" style={{ fontSize: "14px" }}>
                  info
                </span>{" "}
              ...
              <button
                ...
                className="pill-button amber active:scale-95"
              >
                <span className="material-icons" style={{ fontSize: "14px" }}>
                  settings
                </span>{" "}
                ...
                <div className="apple-glass-card rounded-[1.5rem] p-5 flex flex-col justify-center items-start">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span
                      ...
                      style={{ color: "var(--text-primary)" }}
                    >
                      {currentFlowRateNumber > 0
                        ? currentFlowRateNumber.toFixed(1)
                        : "—"}
                    </span>
                    <span
                ...
                <div className="apple-glass-card rounded-[1.5rem] p-5 flex flex-col justify-center overflow-hidden">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center text-[10px]">
                      <span className="font-bold truncate pr-2 text-red-500">
                        Valve Closed
                      </span>
                      <span className="text-slate-400 shrink-0">10:45 AM</span>
                    </div>
                    <div className="flex justify-between items-center text-[10px]">
                      <span className="font-bold truncate pr-2 text-green-500">
                        Valve Opened
                      </span>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {[
                  {
                    label: "ESP32 Email",
                    value: deviceConfig?.esp32_email || "N/A",
                  },
                  {
                    label: "ESP32 Password",
                    value: deviceConfig?.esp32_password || "N/A",
                  },
                ].map(({ label, value }) => (
                  <div
                    key={label}
            ...
            >
              <div className="px-5 py-6">
                  ...
                  {[
                    {
                      ...
                      value: localTsChannel,
                      setter: setLocalTsChannel,
                      placeholder: "Channel ID",
                    },
                    {
                      label: "Read API Key",
                      value: localTsReadKey,
                      setter: setLocalTsReadKey,
                      placeholder: "Read API Key",
                    },
                    {
                      label: "Total Field Number",
                      value: localTotalField,
                      setter: setLocalTotalField,
                      placeholder: "field1, field2...",
                    },
                    {
                      label: "Total Field Name",
                      value: localTotalFieldName,
                      setter: setLocalTotalFieldName,
                      placeholder: "e.g. Reading_7",
                    },
                    {
                      label: "Flow Field Number",
                      value: localFlowField,
                      setter: setLocalFlowField,
                      placeholder: "field1, field2...",
                    },
                    {
                      label: "Flow Field Name",
                      value: localFlowFieldName,
                      setter: setLocalFlowFieldName,
                      placeholder: "e.g. flow rate after filtering",
                    },
                  ].map(({ label, value, setter, placeholder }) => (
                    <div
                      key={label}
                      className="flex items-center justify-between bg-[var(--card-bg)] border rounded-md px-4 py-3 shadow-sm"
                ...
                {[
                  ...
                  <div
                    key={label}
                    className="flex items-center justify-between bg-[var(--card-bg)] border rounded-md px-4 py-3 shadow-sm mt-3"
                    style={{ borderColor: "var(--card-border)" }}
                  >
                    <div
                      className="text-sm font-medium"
              ...
              {saveError && (
                <p
                  className="text-[11px] font-bold text-center mt-3 mb-0"
                  style={{ color: "#FF3B30" }}
                >
                  {saveError}
                </p>
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-[var(--text-muted)] mb-8">
                ...
                <strong>{deviceInfo?.label || hardwareId}</strong> and all its
                historical telemetry data. This action cannot be undone.
              </p>
              <div className="flex flex-col gap-3">
                <button
