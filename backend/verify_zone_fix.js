...
if (!admin.apps.length) {
  try {
    ...
    if (serviceAccountJson) {
      ...
      console.error("❌ Firebase credentials not found in root env file");
      process.exit(1);
    }
  } catch (err) {
...
(async () => {
  try {
    ...
    snapshot.forEach((doc, index) => {
      ...
      console.log(
        `  🌍 Zone ID: ${hasZoneId ? `✅ "${data.zone_id}"` : "❌ MISSING/EMPTY"}`,
      );
      console.log("");
    ...
    if (err.message.includes("Cannot find module")) {
      console.log(
        "ℹ️  Try running this from the backend directory: cd backend && node verify_zone_fix.js",
      );
    }
  } finally {
    process.exit(0);
