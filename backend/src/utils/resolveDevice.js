...
async function resolveDevice(id) {
    ...
    logger.debug(`[resolveDevice] Looking for device: "${id}"`);

    // 1. Try direct document lookup (fastest — indexed by default)
    logger.debug(`[resolveDevice] Attempt 1: Direct doc lookup`);
    const directDoc = await db.collection("devices").doc(id).get();
    if (directDoc.exists) {
        logger.debug(`[resolveDevice] ✅ Found by direct ID`);
        return directDoc;
    }
    logger.debug(`[resolveDevice] ❌ Not found by direct ID`);

    // 2. Query by device_id field (human-readable hardware ID)
    logger.debug(`[resolveDevice] Attempt 2: Query by device_id field`);
    const q1 = await db.collection("devices").where("device_id", "==", id).limit(1).get();
    if (!q1.empty) {
        logger.debug(`[resolveDevice] ✅ Found by device_id: ${q1.docs[0].id}`);
        return q1.docs[0];
    }
    logger.debug(`[resolveDevice] ❌ Not found by device_id`);

    // 3. Fallback to node_id
    logger.debug(`[resolveDevice] Attempt 3: Query by node_id field`);
    const q2 = await db.collection("devices").where("node_id", "==", id).limit(1).get();
    if (!q2.empty) {
        logger.debug(`[resolveDevice] ✅ Found by node_id: ${q2.docs[0].id}`);
        return q2.docs[0];
    }
    logger.debug(`[resolveDevice] ❌ Not found by node_id`);

    logger.debug(`[resolveDevice] ❌ DEVICE NOT FOUND: "${id}"`);
    return null;
}
