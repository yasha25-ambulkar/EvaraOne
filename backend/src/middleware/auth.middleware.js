...
const findSuperadminByUidOrEmail = async (uid, email) => {
  let userDoc = await db.collection("superadmins").doc(uid).get();
  if (userDoc.exists) {
    return userDoc.data();
  }
  ...
  if (email) {
    ...
    if (!emailMatches.empty) {
      return emailMatches.docs[0].data();
    }
  }
...
const requireFirebaseIdentity = async (req, res, next) => {
  ...
  try {
    ...
    logger.error(
      { err: error, url: req.originalUrl },
      "Firebase identity verification failed",
    );
  ...
  try {
    ...
    if (!userData) {
      ...
      const firestoreLookup = async (attempt = 1) => {
        const firestoreTimeout = new Promise((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(`Firestore lookup timed out (attempt ${attempt})`),
              ),
            8000,
          ),
        );
        ...
        const lookupTask = (async () => {
          try {
            // Priority 1: Superadmins by ID, then by email
            const superadminData = await findSuperadminByUidOrEmail(
              decodedToken.uid,
              decodedToken.email,
            );
            if (superadminData) return superadminData;

            // Priority 2: Users collection
      ...
      try {
        ...
        logger.error("Firestore lookup failed after retry", null, {
          category: "auth",
    ...
    if (!userData || !userData.role) {
      return res
        .status(403)
        .json({ error: "Access denied: user account not found in system" });
    }
    ...
    req.user = {
      ...
      role: role,
      display_name:
        userData.display_name || userData.full_name || decodedToken.name,
      community_id: userData.community_id || "",
      customer_id:
        userData.customer_id || userData.uid || userData.id || decodedToken.uid,
    };
  ...
  try {
    ...
    if (!options.bypassCache) {
      ...
      if (cached) {
        ...
        if (
          communityId &&
          (cached.customer_id === communityId ||
            cached.community_id === communityId)
        )
          return true;
      }
    }
    ...
    if (
      communityId &&
      (ownerId === communityId || ownerCommunityId === communityId)
    )
      return true;
...
module.exports = { requireAuth, requireFirebaseIdentity, checkOwnership };
