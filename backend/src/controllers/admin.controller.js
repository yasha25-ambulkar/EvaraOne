...
} = require("../utils/cacheVersioning.js");
...
function isDeviceOnline(device) {
  const status = normalizeStatus(
    device.status || device.online_status || device.operational_status,
  );
...
exports.createZone = async (req, res) => {
  try {
    const { zoneName, state, country, zone_code, description } = req.body;
    ...
    req.log?.error(
      ...
      "[AdminController] Create zone error",
    );
...
exports.getZones = async (req, res) => {
  try {
    ...
    if (req.user.role !== "superadmin") {
      const tenantId =
        req.user.community_id || req.user.customer_id || req.user.uid;
...
exports.getZoneById = async (req, res) => {
  try {
    ...
    if (!zoneOwner) {
      logger.warn(
        `[Tenant Isolation] Zone ${req.params.id} has no owner — rejecting access`,
      );
...
exports.updateZone = async (req, res) => {
  try {
    ...
    req.log?.error(
      { error: error.message },
      "[AdminController] Update zone error",
    );
  ...
  try {
    ...
    if (req.user.role !== "superadmin") {
      const tenantId =
        req.user.community_id || req.user.customer_id || req.user.uid;
      const zoneOwner =
        zoneData.owner_customer_id ||
        zoneData.customer_id ||
        zoneData.tenant_id;
    ...
    req.log?.error(
      { error: error.message },
      "[AdminController] Delete zone error",
    );
...
exports.createCustomer = async (req, res) => {
  try {
    ...
    req.log?.info(
      ...
      "[createCustomer] request received",
    );
    ...
    req.log?.info(
      { uid: firebaseUser.uid, zone_id: finalZoneId },
      "[createCustomer] customer document prepared",
    );
    ...
    req.log?.info(
      { customerDocId: doc.id, zone_id: finalZoneId },
    ...
    req.log?.error(
      { error: error.message },
      "[AdminController] Create customer error",
    );
    res
      .status(500)
  ...
  try {
    ...
    const cacheKey =
      req.user.role === "superadmin"
        ? `user:admin:customers:${cacheParams}`
    ...
    logger.debug(`[AdminController] getCustomers query:`, {
      zone_id,
      community_id,
      role: req.user.role,
    });
    ...
    if (req.user.role === "superadmin" && zone_id) {
      logger.debug(
        `[AdminController] Checking regionFilter fallback for zone_id: ${zone_id}`,
      );

      // Try zoneId (camelCase)
      const zoneIdSnapshot = await db
        .collection("customers")
        .where("zoneId", "==", zone_id.trim())
        .limit(limitStr)
        .get();
      if (!zoneIdSnapshot.empty) {
        logger.debug(
          `[AdminController] Found ${zoneIdSnapshot.size} customers via zoneId fallback`,
        );
      ...
      const regionSnapshot = await db
        .collection("customers")
        .where("regionFilter", "==", zone_id.trim())
        .limit(limitStr)
        .get();
      if (!regionSnapshot.empty) {
        logger.debug(
          `[AdminController] Found ${regionSnapshot.size} customers via regionFilter fallback`,
        );
        // Deduplicate by ID
        const existingIds = new Set(customers.map((c) => c.id));
      ...
      customers.map(async (customer) => {
        try {
          ...
          const onlineDeviceCount =
            customerDevices.filter(isDeviceOnline).length;
          ...
          return {
            ...
            devices: customerDevices.map((device) => ({
              ...
              node_key:
                ...
                null,
              location:
                device.location || device.zoneName || device.zone_name || null,
            })),
            deviceCount,
            onlineDeviceCount,
          ...
          logger.warn(
            `[AdminController] Failed to enrich customer ${customer.id}:`,
            err.message,
          );
          return {
            ...customer,
...
exports.getCustomerById = async (req, res) => {
  try {
    ...
    const customerIds = Array.from(
      new Set(
        [customer.id, customer.uid, customer.firebase_uid, customer.customer_id]
          .filter(Boolean)
          .map((v) => String(v)),
      ),
    );
    ...
    const customerDevices = devicesSnap.docs
      ...
      .filter((device) =>
        customerIds.includes(
          String(
            device.customer_id || device.customerId || device.customerID || "",
          ),
        ),
      );
...
exports.updateCustomer = async (req, res) => {
  try {
    ...
    if (authUid) {
      ...
      if (normalizedData.display_name || normalizedData.full_name) {
        authPayload.displayName =
          normalizedData.display_name || normalizedData.full_name;
      }
    ...
    req.log?.error(
      { error: error.message },
      "[AdminController] Update customer error",
    );
...
exports.deleteCustomer = async (req, res) => {
  try {
    ...
    req.log?.error(
      { error: error.message },
      "[AdminController] Delete customer error",
    );
...
exports.createNode = async (req, res) => {
  ...
  try {
    ...
    const isValveDevice =
      typeNormalized === "evaravalve" || typeNormalized === "valve";
    ...
    if (isValveDevice) {
      ...
      if (!devicePassword) {
        devicePassword = generateDeviceSecret();
      }
    }
    ...
    if (isValveDevice) {
      try {
        ...
        logger.debug(
          `[createNode] ✅ Created Firebase Auth user for valve device: ${deviceEmail}`,
        );
      } catch (authError) {
        if (authError.code === "auth/email-already-exists") {
          firebaseUser = await admin.auth().getUserByEmail(deviceEmail);
          logger.debug(
            `[createNode] ℹ️ Reused existing Firebase Auth user for valve device: ${deviceEmail}`,
          );
        } else {
          throw new AppError(authError.message, 400);
        }
    ...
    if (typeNormalized === "evaratank" || typeNormalized === "tank") {
      ...
      deviceData.sensor_field_mapping = {
        [levelField]: "water_level_raw_sensor_reading",
      };
      ...
      const selectedTotalField = (
        total_volume_field ||
        req.body.totalVolumeField ||
        "field1"
      ).trim
      ...
      deviceData.flow_field_name =
        flow_field_name ||
        req.body.flow_field_name ||
        req.body.flowFieldName ||
        "";
      deviceData.total_volume_field = selectedTotalField;
      deviceData.total_volume_field_name =
        total_volume_field_name ||
        req.body.total_volume_field_name ||
        req.body.totalVolumeFieldName ||
        "";
      deviceData.fields = {
        flow: selectedFlowField,
        total_volume: selectedTotalField,
      };
    ...
    logger.debug(
      `[createNode] 📝 Creating single document in devices/${idForDevice}`,
    );
    ...
    if (verifyDoc.exists) {
      logger.debug(
        `[createNode] ✅ VERIFIED: Document exists in devices/${idForDevice}`,
      );
    } else {
    ...
    if (thingspeakChannelId && thingspeakReadKey) {
      logger.debug(
        `[createNode] 🔄 Fetching and saving channel metadata for device ${idForDevice}`,
      );
      try {
        const {
          fetchAndSaveChannelMetadata,
        } = require("../services/channelMetadataService.js");
        ...
        if (channelMeta) {
          ...
          logger.warn(
            `[createNode] ⚠️  Channel metadata fetch returned null (non-fatal)`,
          );
        }
      } catch (metaErr) {
        logger.warn(
          `[createNode] ⚠️  Failed to fetch/save channel metadata (non-fatal):`,
          metaErr.message,
        );
      }
    }
    ...
    try {
      if (global.io) {
        ...
        logger.debug(
          `[Device] 📡 Emitted device:added to customer ${customerId}`,
        );
      }
    } catch (socketErr) {
      logger.warn(
        "[Device] Socket emission failed (non-fatal):",
        socketErr.message,
      );
    }
    ...
    if (firebaseUser?.uid && createdFirebaseUser) {
      try {
        ...
        logger.warn(
          "[createNode] ⚠️ Failed to clean up Firebase Auth user after node creation error:",
          cleanupErr.message,
        );
      }
    }
...
exports.getNodes = async (req, res) => {
  try {
    logger.debug(
      `[AdminController] getNodes for user:`,
      req.user.uid,
      "role:",
      req.user.role,
    );
    ...
    for (const batch of typeBatches) {
      for (const item of batch) {
        ...
        let analyticsTemplate =
          registryData.analytics_template || meta.analytics_template;
    ...
    logger.debug(
      `[AdminController] Final devices with analytics_template:`,
      devices.map((d) => ({
        id: d.id,
        type: d.device_type,
        template: d.analytics_template,
      })),
    );
...
exports.updateNode = async (req, res) => {
  try {
    ...
    if (!type)
      return res.status(400).json({ error: "Device type not specified" });
    ...
    if (body.displayName || body.label)
      metaUpdate.label = trimmed(body.displayName || body.label);
    if (body.deviceName || body.device_name)
      metaUpdate.device_name = trimmed(body.deviceName || body.device_name);

    // ThingSpeak credentials (flexible naming)
    const readKey =
      body.thingspeakReadKey ||
      body.thingspeak_read_key ||
      body.thingspeak_read_api_key;
    ...
    if (body.longitude !== undefined)
      metaUpdate.longitude = parseNumberSafely(body.longitude);
    ...
    if (type === "evaratank" || type === "tank") {
      const cap =
        ...
        body.capacity_liters_override;
      if (cap !== undefined) metaUpdate.tank_size = parseFloat(cap) || 0;

      const config = {};
      const depthVal =
        body.depth || body.height_m || body.max_depth || body.tank_height;
      ...
      if (field) {
        metaUpdate.sensor_field_mapping = {
          [trimmed(field)]: "water_level_raw_sensor_reading",
        };
      }
      ...
      if (depthVal !== undefined)
        config.total_depth = parseFloat(depthVal) || 0;

      const stat =
        body.staticDepth || body.static_water_level || body.static_depth;
      if (stat !== undefined) config.static_water_level = parseFloat(stat) || 0;

      const dyn =
        body.dynamicDepth || body.dynamic_water_level || body.dynamic_depth;
      if (dyn !== undefined) config.dynamic_water_level = parseFloat(dyn) || 0;

      const thres = body.rechargeThreshold || body.recharge_threshold;
      if (thres !== undefined)
        config.recharge_threshold = parseFloat(thres) || 0;

      if (Object.keys(config).length > 0) metaUpdate.configuration = config;

      const field =
        body.borewellDepthField || body.water_level_field || body.depth_field;
      if (field) {
        metaUpdate.sensor_field_mapping = {
          [trimmed(field)]: "water_level_in_cm",
        };
      }
    } else if (type === "evaraflow") {
      const config = {};
      if (body.maxFlowRate || body.max_flow_rate)
        config.max_flow_rate =
          parseFloat(body.maxFlowRate || body.max_flow_rate) || 0;
      if (Object.keys(config).length > 0) metaUpdate.configuration = config;

      if (
        body.flowRateField ||
        body.meterReadingField ||
        body.flow_rate_field ||
        body.meter_reading_field
      ) {
        ...
        if (!rateField)
          rateField =
            Object.keys(currentMap).find(
              (k) => currentMap[k] === "flow_rate",
            ) || "field2";
        ...
        if (!readingField)
          readingField =
            Object.keys(currentMap).find(
              (k) => currentMap[k] === "current_reading",
            ) || "field1";
      ...
      if (body.configuration) {
        if (body.configuration.min_threshold !== undefined)
          config.min_threshold =
            parseFloat(body.configuration.min_threshold) || 0;
        if (body.configuration.max_threshold !== undefined)
          config.max_threshold =
            parseFloat(body.configuration.max_threshold) || 2000;
      }
      if (Object.keys(config).length > 0) metaUpdate.configuration = config;
      ...
      if (body.temperature !== undefined)
        metaUpdate.temperature = parseFloat(body.temperature) || 0;
      if (body.waterQualityRating)
        metaUpdate.waterQualityRating = trimmed(body.waterQualityRating);
      ...
      const config = {};
      if (body.configuration) {
        if (body.configuration.voltage_max !== undefined)
          config.voltage_max =
            parseFloat(body.configuration.voltage_max) || 500;
        if (body.configuration.current_max !== undefined)
          config.current_max =
            parseFloat(body.configuration.current_max) || 100;
      }
      if (body.voltage_max !== undefined || body.voltageMax !== undefined) {
        config.voltage_max =
          parseFloat(body.voltage_max || body.voltageMax) || 500;
      }
      if (body.current_max !== undefined || body.currentMax !== undefined) {
        config.current_max =
          parseFloat(body.current_max || body.currentMax) || 100;
      }
      if (Object.keys(config).length > 0) metaUpdate.configuration = config;
      ...
      if (
        ...
        body.powerField ||
        body.frequencyField ||
        body.waterLevelField ||
        body.voltage_field ||
        body.current_field ||
        body.power_field ||
        body.frequency_field ||
        body.water_level_field
      ) {
        const docData = (await metaRef.get()).data() || {};
        const currentMap = docData.sensor_field_mapping || {};
        ...
        if (lvlF) {
          ...
          const existingLvl = Object.keys(currentMap).find(
            (k) => currentMap[k] === "water_level_raw_sensor_reading",
          );
          if (existingLvl)
            newMapping[existingLvl] = "water_level_raw_sensor_reading";
        }
      ...
      if (body.voltageValue !== undefined)
        metaUpdate.voltageValue = parseFloat(body.voltageValue) || 0;
      if (body.currentValue !== undefined)
        metaUpdate.currentValue = parseFloat(body.currentValue) || 0;
      if (body.powerValue !== undefined)
        metaUpdate.powerValue = parseFloat(body.powerValue) || 0;
      if (body.frequencyValue !== undefined)
        metaUpdate.frequencyValue = parseFloat(body.frequencyValue) || 50;
      if (body.powerFactor !== undefined)
        metaUpdate.powerFactor = parseFloat(body.powerFactor) || 1.0;
      if (body.status) metaUpdate.status = trimmed(body.status);
      metaUpdate.lastUpdated = new Date();
      ...
      const config = {};
      if (body.configuration) {
        if (body.configuration.max_position !== undefined)
          config.max_position =
            parseFloat(body.configuration.max_position) || 100;
      }
      if (Object.keys(config).length > 0) metaUpdate.configuration = config;
      ...
      const selectedFlowField =
        ...
        "field2";
      const selectedTotalField =
        body.total_volume_field || body.totalVolumeField || "";
      if (selectedFlowField) {
        ...
        if (body.flow_field_name || body.flowFieldName) {
          metaUpdate.flow_field_name = trimmed(
            body.flow_field_name || body.flowFieldName,
          );
        }
        ...
        if (selectedTotalField) {
          ...
          if (body.total_volume_field_name || body.totalVolumeFieldName) {
            metaUpdate.total_volume_field_name = trimmed(
              body.total_volume_field_name || body.totalVolumeFieldName,
            );
          }
        }
        ...
        metaUpdate.sensor_field_mapping = {
          ...
          ...(selectedTotalField
            ? { [trimmed(selectedTotalField)]: "totalVolume" }
            : {}),
        };
      }
      ...
      if (body.flow_field_name === "")
        metaUpdate.flow_field_name = admin.firestore.FieldValue.delete();
      if (body.location !== undefined)
        metaUpdate.location = trimmed(body.location);
      if (body.status) metaUpdate.status = trimmed(body.status);
      metaUpdate.lastUpdated = new Date();
    }
    ...
    const customerId =
      metaUpdate.customer_id ||
      deviceDoc.data().customer_id ||
      deviceDoc.data().customerId;
    if (customerId && global.io) {
      ...
      logger.debug(
        `[AdminController] ✅ device:updated event emitted for device: ${deviceDoc.id}, customer: ${customerId}`,
      );
    }
...
exports.deleteNode = async (req, res) => {
  try {
    ...
    if (customerId && global.io) {
      ...
      logger.debug(
        `[AdminController] ✅ device:deleted event emitted for device: ${deviceId}, customer: ${customerId}`,
      );
    }
  ...
  try {
    ...
    const [zonesVer, customersVer, devicesVer] = await Promise.all([
      db
        ...
        .doc("zones")
        .get()
        .then((d) => (d.exists ? d.data().version : 1))
        .catch(() => 1),
      db
        .collection("_cache_versions")
        .doc("customers")
    ...
    const zoneStats = zones.map((zone) => {
      const zoneCustomers = customers.filter(
        (c) =>
          c.zone_id === zone.id ||
          c.zoneId === zone.id ||
...
exports.getDashboardInit = async (req, res) => {
  try {
    ...
    const [zonesRes, nodesRes, summaryRes] = await Promise.all([
      new Promise((resolve) =>
        exports.getZones(req, { status: () => ({ json: resolve }) }),
      ),
      new Promise((resolve) =>
        exports.getNodes(req, { status: () => ({ json: resolve }) }),
      ),
      new Promise((resolve) =>
        exports.getDashboardSummary(req, { status: () => ({ json: resolve }) }),
      ),
    ]);
...
exports.updateSystemConfig = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "System configuration updated" });
  } catch (error) {
...
exports.updateDeviceVisibility = async (req, res) => {
  try {
    ...
    if (typeof isVisibleToCustomer !== "boolean") {
      return res.status(400).json({
        error: "isVisibleToCustomer must be a boolean (true or false)",
      });
    }
    ...
    if (customerId && global.io) {
      ...
      logger.debug(
        `[AdminController] ✅ device:updated event emitted for visibility change: ${deviceDoc.id}`,
      );
    }

    logger.debug(
      `[AdminController] Device ${id} visibility set to: ${isVisibleToCustomer}`,
    );
    ...
    return res
      .status(500)
      .json({ error: "Failed to update device visibility" });
  }
};
...
exports.updateDeviceParameters = async (req, res) => {
  try {
    ...
    if (
      !customer_config ||
      typeof customer_config !== "object" ||
      Array.isArray(customer_config)
    ) {
      return res
        .status(400)
        .json({ error: "customer_config must be a valid object" });
    }
    ...
    if (customerId && global.io) {
      ...
      logger.debug(
        `[AdminController] ✅ device:updated event emitted for parameter changes: ${deviceDoc.id}`,
      );
    }

    logger.debug(
      `[AdminController] Device ${id} parameters updated:`,
      customer_config,
    );
