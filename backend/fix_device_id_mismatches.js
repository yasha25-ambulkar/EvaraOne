...
async function findMismatches() {
  ...
  for (const registryDoc of registrySnapshot.docs) {
    ...
    if (deviceType === "evaratank" || deviceType === "tank")
      targetCollection = "evaratank";
    else if (deviceType === "evaradeep" || deviceType === "deep")
      targetCollection = "evaradeep";
    else if (deviceType === "evaraflow" || deviceType === "flow")
      targetCollection = "evaraflow";
    else if (deviceType === "evaratds" || deviceType === "tds")
      targetCollection = "evaratds";
    ...
    if (expectedMetaDoc.exists) {
      ...
      console.log(
        `✅ [${targetCollection}] ${registryId}: Metadata at correct location (label: ${expectedMetaDoc.data().label})`,
      );
    } else {
      // ❌ MISMATCH: Metadata missing at expected location - search for it
      console.log(
        `⚠️  [${targetCollection}] ${registryId}: NO metadata at expected location, searching...`,
      );
      ...
      for (const metaDoc of allMetaSnapshot.docs) {
        ...
        if (
          meta.device_id === registry.device_id ||
          (meta.created_at === registry.created_at &&
            meta.label === registry.label)
        ) {
          console.log(
            `   🔴 FOUND MISMATCH: Metadata ID "${metaDoc.id}" != Registry ID "${registryId}"`,
          );
      ...
      if (!foundMismatched) {
        console.log(
          `   ❌ ORPHANED REGISTRY: No metadata found in ${targetCollection} collection!`,
        );
        mismatches.push({
          registryId,
          registryRef: registryDoc,
...
async function fixMismatches(mismatches) {
  ...
  for (const mismatch of mismatches) {
    ...
    if (!mismatch.metaData) {
      console.log(
        `⏭️  [SKIP] ${mismatch.registryId}: No metadata to copy (orphaned registry)`,
      );
      continue;
    }

    console.log(
      `🔧 [FIX] ${mismatch.registryId}: Copying metadata from ${mismatch.metaId} → ${mismatch.registryId}`,
    );

    // Copy metadata to correct location
    const newMetaRef = db
      .collection(mismatch.targetCollection)
      .doc(mismatch.registryId);
...
async function run() {
  try {
    ...
    console.log(
      "║         DEVICE ID MISMATCH FIX - DIAGNOSTIC & HEAL         ║",
    );
    console.log(
      "╚════════════════════════════════════════════════════════════╝",
    );
    ...
    console.log(
      `   With metadata (fixable): ${mismatches.filter((m) => m.metaData).length}`,
    );
    console.log(
      `   Orphaned (no metadata): ${mismatches.filter((m) => !m.metaData).length}`,
    );
